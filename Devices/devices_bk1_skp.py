...
def tagDataskp(dList, position):
    """
        Toma una posición para obtener de la lista dList.
    """
    try:
        tagdata = dList[position]
    except: sys.stderr.write("Error al obtener el Tag Data")
...
class SKPDevice(Device):
    ...
    tagDataSKP = {  # (position, function_tagData, function_convert )
                    "id"        : (2, tagDataskp, None), # ID de la unidad
                    "type"      : (0, tagDataskp, None),
                    "typeEvent" : (3, tagDataskp, None), # 
                    "codEvent"  : (1, tagDataskp, None), # Codigo de evento activado (en Antares de 00 a 49, en e.Track de 00 a 99)
                    "weeks"     : (0, tagDataskp, None), # Es el numero de semanas desde 00:00AM del 6 de enero de 1980.
                    "dayWeek"   : (0, tagDataskp, None), # 0=Domingo, 1=Lunes, etc hasta 6=sabado.
                    "time"      : (4, tagDataskp, None), # Hora expresada en segundos desde 00:00:00AM
                    "lat"       : (6, tagDataskp, None), # Latitud
                    #"lat"       : (6, tagDataskp, latWgs84ToDecimal),    # Latitud
                    "lng"       : (8, tagDataskp, None), # Longitud
                    #"lng"       : (23, 9, 0, tagDataskp, lngWgs84ToDecimal),    # Longitud
                    "speed"     : (10, tagDataskp, MphToKph),   # Velocidad en MPH
                    "course"    : (11, tagDataskp, None), # Curso en grados
                    "gpsSource" : (0, tagDataskp, None), # Fuente GPS. Puede ser 0=2D GPS, 1=3D GPS, 2=2D DGPS, 3=3D DGPS, 6=DR, 8=Degraded DR. # Problema DB si no son enteros    
                    "ageData"   : (0, tagDataskp, None), # Edad del dato. Puede ser 0=No disponible, 1=viejo (10 segundos) ó 2=Fresco (menor a 10 segundos) # Problema DB si no son enteros
                    "odometro"  : (15, tagDataskp, None) # Odómetro
                 }
    ...
    def __parse(self, data):
        ...
        try:
            ...
            # ['', '5', 'SKP87', '$GPRMC', '122408.00', 'A', '0441.935953', 'N', '07404.450302', 'W', '0.0', '0.0', '180912', '5.5', 'E', 'A*2F']
            #
            for tag, (position, parseFunc, convertFunc) in self.tagDataSKP.items():
                self[tag] = convertFunc and convertFunc(parseFunc(dataList, position)) or parseFunc(dataList, position)
